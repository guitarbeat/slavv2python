...
## Decision
We have transitioned to "Deep" Schema classes (`EnergyResult`, `VertexSet`, `EdgeSet`, `NetworkResult`) that provide high leverage through a small interface.
...
- **Backward Compatibility**: To support existing downstream analytics, the classes retain a `.to_dict()` method, though its use is discouraged for internal pipeline handoffs.
