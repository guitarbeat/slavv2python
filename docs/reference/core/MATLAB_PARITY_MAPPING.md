...
| `add_vertices_to_edges.m` | `slavv_python/pipeline/edges/bridge_insertion.py`, `slavv_python/pipeline/edges/manager.py` | Source-aligned | Bridge insertion downstream of edges; proven through network assembly. |
| `get_network_V190.m`, `sort_network_V180.m`, `get_strand_objects.m` | `slavv_python/pipeline/network/manager.py`, `slavv_python/pipeline/network/construction.py`, `slavv_python/pipeline/network/operations.py` | Source-aligned; certified per [ADR 0012](../../adr/0012-edge-watershed-parity-bar.md) | Strand assembly, bifurcation multiset, and trace geometry certified under ADR 0012 on full volume (`canonical_full_v18`). Historical crop stage isolation (10,722/10,722 strands, 5,601/5,601 bifurcations) verified topology. **Live status**: [ONE TRUTH](EXACT_PROOF_FINDINGS.md#one-truth--phase-1-parity-validated-from-disk). |
...
1. `slavv_python/pipeline/edges/watershed/matlab_*.py` ports aligned with the released MATLAB
