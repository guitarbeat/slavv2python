...
| `get_network_V190.m`, `sort_network_V180.m`, `get_strand_objects.m` | `slavv_python/pipeline/network/manager.py`, `slavv_python/pipeline/network/construction.py`, `slavv_python/pipeline/network/operations.py` | **Topology CERTIFIED EXACT** ([ADR 0012](../../adr/0012-edge-watershed-parity-bar.md)) | Fed curated edges+vertices: strands 10,722/10,722 (endpoint-pair multiset), bifurcations 5,601/5,601 (0 missing/extra). Strand-assembly orientation + scale fixes landed; geometry residual is a sub-voxel strand-smoothing floor under trace tolerance. |
