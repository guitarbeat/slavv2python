...
| **Phase 1 Closure** 🤖 | Claimed when a **new** Claim Run Root for full `180709_E` passes **evaluated** ADR 0012 for **both** Edges and Network (after Energy/Vertices pass). Network ADR 0012 pass closes Phase 1; strict-field remains stretch. If Network still fails after the ranking fix, deepen ranking/provenance first—do not rewrite Network by default. Definition only—**live whether closed** is [EXACT_PROOF_FINDINGS](EXACT_PROOF_FINDINGS.md); operator brief: [.claude/HANDOFF.md](../../../.claude/HANDOFF.md). |
