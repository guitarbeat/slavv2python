...
| **Candidate Set** | Authoritative Candidates from Edge Discovery; input to Edge Selection. Mid-stage Edges Checkpoint (not a Stage Result). Code: `CandidateManifest`. Production write: `04_Edges/candidates.pkl`. Former `checkpoint_edge_candidates.pkl` is a read-only fallback. |
...
| **Claim Run Root** 🤖 | Canonical full-volume Parity Run directory named in [ONE TRUTH](EXACT_PROOF_FINDINGS.md#one-truth--phase-1-parity-validated-from-disk) as the live Certification claim surface. After a parity-sensitive fix open a **new** claim root. Historical proofs may live under `workspace/reports/phase1_volume_archive/`. Live root ID only in ONE TRUTH. |
| **Experiment Root** 🤖 | On-disk home of Oracles, datasets, live Claim Run Root / crop guard / stretch dest, and archived proofs under `workspace/`. GitHub carries code + Vectorization-Public; binaries stay local or USB/rsync (not GitHub LFS). `workspace/scratch/` is gitignored. Check: `slavv parity inspect-experiment-root` (rejects LFS pointer stubs). Not MATLAB source or neurovasc-db. Freeze JSON `do_not_overwrite` / `LIVE_DEST_NAMES` is live dests; `PROTECTED_DEST_NAMES` is the writer blocklist — do not equate them. |
...
| **True Zero-Tolerance Stretch** 🤖 | Extra post–Phase 1 goal: every compared MATLAB↔Python number identical bits (including Energy), not just “close.” Default allclose is not this bar and is not 100%. Live status: findings stretch subsection + dest `stretch_status.json`, not ONE TRUTH. |
| **Stretch Status Taxonomy** 🤖 | Labels: `blocked_float_path` (Energy last digits still differ) / `incomplete_discrete` / `incomplete_infra` (could not finish the compare) / `incomplete_at_full` / `stretch_complete` (Energy **and** discrete bit-equal at full). Infra ≠ `blocked_float_path`. Allclose ≠ `stretch_complete`. |
| **Stretch Unlock Token** 🤖 | Crop file saying “this field set is bit-equal on crop, so full stretch may start.” U5/U6 stay gated without it. |
...
| **Exact Proof Findings** 🤖 | Live status log for exact-parity work. Authoritative section: [ONE TRUTH](EXACT_PROOF_FINDINGS.md#one-truth--phase-1-parity-validated-from-disk) (pass/fail, claim root, residual). Also holds compound-solution index and audit inventory. |
