...
2. **Phase 1 ship gate = Network ADR 0012** — Edges ADR 0012 ✅ PASS evaluated through `canonical_full_v10`. Network ❌ FAIL (`v10`: 48,583 / 48,049 strands), **entirely downstream of the edge-set mismatch** (MATLAB-edge isolation is exact). Crop final metrics have moved to one pair swap; the next planning choice is single-swap triage versus a successor full root.
