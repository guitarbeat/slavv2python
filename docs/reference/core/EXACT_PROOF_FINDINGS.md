...
**Last Updated**: 2026-06-29 (oracle v2; **Energy + Vertices CERTIFIED** (crop) under ADR 0011 gate; **Edges: orientation bug fixed, residual is watershed order-sensitivity → ADR 0012 ownership-map bar**; **Canonical full energy does NOT yet certify** — resume-orientation bug fixed, but a real **multi-chunk downsampled-octave** divergence remains (0.235% scale mismatches, octaves 3–4; `n_jobs` and seams ruled out) — see [canonical-energy-high-octave-divergence](../../solutions/parity/canonical-energy-high-octave-divergence.md))
...
| **Edges** | v29 ~88.7% pair match (baseline) | 🟡 **Orientation double-transpose FIXED; residual is emergent watershed order-sensitivity, not a local bug → certify on ownership-map + trace tolerance (see RESOLVED at row end).** Current: **63.5% voxel-ownership agreement** with MATLAB on the correct grid. _History:_ Partially closed (~52% shared). Fixes `edge_number_tolerance`→2 and exact-route conflict-painting-off moved Python **9,429 → 13,775** (vs MATLAB 15,511). **Frontier-ordering ruled out** (faithful sorted-list closed only ~3% at 10× cost; reverted). **Diagnosis (per-edge, 2026-06-25): early trace termination on long paths** — 5,774/7,164 missing edges are **never traced** (Python over-traces 18,378 candidates but mis-connects), and never-traced edges are systematically longer (mean len 9.1 vs 5.3 shared). **Penalty/distance/tolerance math verified line-equivalent to MATLAB.** **Size-penalty reference fix** (use origin-vertex scale per `get_edges_by_watershed.m:243`, not the current voxel's) applied as a faithfulness correction but **ruled out** (no metric change — scale rarely drifts here; degree cap now matches at 4). **`microns_per_voxel` ordering RULED OUT**: making the forward-direction LUT use `mpv_matlab` (to match the strel) *regressed* shared 8,135→7,437 — the `mpv_matlab` (strel) / raw (forward-LUT) split is intentional/correct. **Three hypotheses now ruled out** (frontier ordering, size-reference, directional frame). Residual is a deep tracing-geometry divergence (Python over-traces ~18.4k candidates but mis-connects long edges); needs single-edge trace instrumentation or a 2-vertex synthetic isolation, not more parameter guesses. **Edges paused** pending that fresh approach. **systematic-debugging Phase 1 (2026-06-25):** catchment-subdivision ruled out as primary — only 15-19% of missing edges have a Python 2-hop `A-C-B` bridge; **81% have no bridge at all** and their endpoints are under-connected (mean degree **2.00 vs MATLAB 2.61**). Python under-claims territory on long paths so catchments never meet. **Root cause LOCATED (instrumentation, 2026-06-25):** captured the watershed `vertex_index_map` (patched `VoxelClaimMap`) and walked 3 long missing edges' MATLAB traces through it. Pattern is unmistakable: **both catchments expand toward each other and stop, leaving the middle of the vessel UNCLAIMED (owner 0)** — e.g. edge (2804,4287) len 53: A owns first 13, then **35 unclaimed**, then B owns last 5. NOT subdivision. So Python's per-step expansion terminates **mid-vessel at high cumulative `d_over_r`** (the farthest point from both vertices) where MATLAB continues; energy matches to 2e-11, so it is the distance/tolerance penalty *applied during expansion*, not the energy field. **Iron-Law next step (no fix yet):** log adjusted-energy + `d_over_r` + raw-energy at the exact termination voxel for one edge and compare Python vs MATLAB to find the divergent penalty term. **ROOT CAUSE CONFIRMED (instrumented, 2026-06-25):** the cumulative distance penalty `exp(-(3*d_over_r/dt)^2/2)` (dt=3 both sides) crushes long half-traces to ~0 before they reach the vessel midpoint, because Python computes `r_over_R`/`d_over_r` with the **origin vertex's radius** instead of the **local vessel radius**. Evidence on edge (2804,4287): vertices are small (scale 1-2, r≈1.5µm) but the vessel is large (native scale 46-72, r 8-23µm); Python's captured `d_over_r`=**14.9** at step 12 matches the origin-radius calc (15.7), not the native-radius calc (**2.6**). So `d_over_r` grows ~6× too fast → penalty annihilates the energy signal → the argmin wanders off-vessel (best move went to an off-trace voxel with adj≈1e-39) → catchments never meet. MATLAB completes these edges, so it uses the larger local radius. **Hypothesis RETRACTED after verification:** MATLAB's `r_over_R = strel_distance / lumen_radius[scale]` (`get_edges_V300.m:331`) is **identical** to Python's, and the cumulative distance penalty `exp(-(3*d_over_r_current/dt)^2/2)` is a **common scalar** over all of the current voxel's neighbors — so it cannot change which neighbor `argmin` selects, and cannot change tolerance (negative raw × positive factors stays negative; frontier priority uses *raw* energy). The `d_over_r`=14.9 figure was real but **causally irrelevant** to edge topology. No fix applied (verification prevented a wrong one). **Actual cause is the per-neighbor move selection** (size / local-distance / direction factors and/or raw-energy ranking): on the instrumented edge the expansion picked an *off-vessel* neighbor over the on-trace one, so mid-vessel voxels are claimed but never expanded. The earlier directional-frame attempt (a per-neighbor direction factor) regressed, so this is subtle. **MATLAB GROUND-TRUTH OBTAINED (R2019a, 2026-06-25):** ran an instrumented standalone `get_edges_V300` on the crop's oracle energy+curated-vertices (single chunk) and dumped the watershed `vertex_index_map`; walked edge (2804,4287)'s trace through MATLAB's ownership vs Python's (axis order disambiguated [z,x,y], 80/80 vertices validate). **At IDENTICAL inputs:** MATLAB's catchment A expands to ~voxel 38 and meets B (only 7/53 unclaimed → edge forms); **Python's A dies at voxel 13 and wanders off-vessel (35/53 unclaimed → no edge).** So the divergence is a **genuine Python watershed expansion bug** — it under-expands / mis-steps vs MATLAB on the same data; NOT inputs, NOT d_over_r (common scalar), NOT a definition issue. Localized to the per-step move selection (size / local-distance `(1-cos r/R)` / direction factors or argmin tie-break in `_matlab_frontier_adjusted_neighbor_energies` / `_argmin_with_linear_index_tiebreak`). **RESOLVED — two findings (2026-06-25):** (1) **Orientation double-transpose bug FIXED** (`fix/edge-watershed-orientation`, commit `e9dcc141`): `generate_watershed_candidates` pre-aligned `[Z,Y,X]→[Y,X,Z]` and then the engine reoriented *again* (`np.transpose(...,(1,2,0))` + mpv `[[1,2,0]]`), so the production frontier path ran on a scrambled `[X,Z,Y]` grid. Proven via global voxel-ownership vs MATLAB's `vertex_index_map` dump: **correct single transpose = 63.47% ownership agreement** (map shape `(64,256,256)`); the old double-transpose collapsed to **1.07%** under the implied Y↔X swap and produced a wrong-shape `(256,256,64)` map. The double-transpose's *higher* raw edge-PAIR overlap (9,098 vs 8,785) was a **coincidental graph artifact** (matching vertex-index pairs while traces ran through spatially wrong voxels), NOT spatial parity — so the old pair-overlap headline was inflated. (2) **Residual reach gap is emergent global-ordering sensitivity, NOT a local bug.** With the correct grid, per-neighbor decomposition at the instrumented divergence (MATLAB R2019a dump of strel `r_over_R`/sizes/adjusted-energies vs Python) shows **`r_over_R` matches MATLAB to 4 decimals every neighbor, sizes match, and the size/local-distance/direction penalty math is faithfully ported**; Python's per-step argmin agrees with MATLAB where both read the same state. The remaining adjusted-energy ratios (1.3–6.8×) reduce to differences in the **shared, mutated `energy_temp` map** — voxels overwritten by other vertices' watersheds (popped earlier) in a subtly different order than MATLAB. The watershed is a greedy shared-state flood-fill: tiny accumulated claim/queue-order differences cascade into different topology. **Decision:** stop hunting a single edge bug; certify edges on the **ownership-map metric (63.5%) + per-edge trace tolerance**, not exact pair-set equality (see [ADR 0012](../../adr/0012-edge-watershed-parity-bar.md)). MATLAB ground-truth harness retained at `workspace/scratch/matlab_edge_instr/` (instrumented `get_edges_V300` + `get_edges_by_watershed` + standalone driver). |
| **Network** | End-to-end pipeline runs | 🟢 **TOPOLOGY EXACT** / 🟡 geometry pending. Fed MATLAB curated edges + curated vertices (stage isolation), Python `construct_network` reproduces MATLAB's `get_network_V190` topology **100%**: strands by endpoint-pair multiset **10,722/10,722**, bifurcations **5,601/5,601** (0 missing/extra). The exact-proof comparator now has a network-specific path ([ADR 0012](../../adr/0012-edge-watershed-parity-bar.md)): strands compared as order-independent endpoint-pair multisets (MATLAB `strands2vertices` stores end-pairs; Python `strands` stores full chains — both reduce to the bounding pair), bifurcations as multisets, per-strand geometry compared after a canonical endpoint-keyed reorder. **Geometry (Phase B, in progress):** `prove-exact --stage network` now fails on `network.strand_subscripts` (no longer on topology). **Scale-subscript off-by-one FIXED** — the strand scale column is a 1-based scale index in both MATLAB raw and the Python output (network smoothing offsets the 0-based edge scale by +1); the comparator's `_normalize_spatial_scale_matrix_list` was over-shifting it by `-1` along with the spatial columns, now shifts only the 3 spatial columns (`array_normalization.py`). Verified median(MATLAB_raw_scale − Python_scale)=0. **Multi-edge assembly off-by-one FIXED** — `_build_graph_state` (`network/base.py`) keys undirected edges by the sorted `(min,max)` vertex pair, and `_graph_state_ordered_edges` returns that sorted pair as the connection, but the stored **trace was not reoriented** to match. For any edge with `start > end`, the reported connection direction then contradicted the trace direction, so `sort_network_V180` (which derives `edge_backwards` from the connection) mis-oriented multi-edge strands, producing junction-duplicate points that the round10 dedup then dropped (−1 per affected strand). Fix: reverse the trace/scale/energy when storing under a flipped key so connection↔trace stay aligned. Result: **strand point counts now match MATLAB 100%** (10,508/10,508 matched strands); the large multi-edge divergence (was p99 ~5, max ~22 voxel) collapsed to sub-voxel. Calling `_matlab_network_topology` directly on the curated edges already gave the exact MATLAB total (101,624 points), confirming the topology/assembly code was correct and the bug was purely in the graph-state layer. **Remaining residual:** sub-voxel **strand smoothing drift** only — single-edge median **0.10** (max 1.26), multi-edge median **0.17** (max 2.06) voxel; a two-pass Gaussian-smoothing kernel floor (sigma `√2/2`, trim, round-half-up dedup all confirmed identical to MATLAB; endpoints forced exact). Well above the 1e-7 `np.allclose` bar (unrealistic for smoothed traces) — geometry certified under a sub-voxel trace tolerance per ADR 0012. |
...
| **Canonical full run** | `workspace/runs/oracle_180709_E/canonical_full_v3` | 🟠 **Energy does NOT certify (open).** Orientation bug FIXED (energy now correct `(64,512,512)`; was `(512,64,512)` from a resume/init double-permute — see [resume-energy-orientation](../../solutions/parity/resume-energy-orientation.md)). With correct orientation the proof runs but energy fails: **`scale_indices` 39,494/16.8M (0.235%) mismatches**, genuine energy diffs (median 1.79), concentrated in **multi-chunk downsampled octaves 3–4** (crop certified because its octave 4 is single-chunk). **`n_jobs` ruled out** (crop n_jobs=1≡6, byte-identical) and **chunk seams ruled out** (mismatches are chunk-*interior*). Next: MATLAB octave-4 ground-truth (computation-vs-selection). Full characterization: [canonical-energy-high-octave-divergence](../../solutions/parity/canonical-energy-high-octave-divergence.md). v4 rerun paused (would reproduce; deterministic). |
