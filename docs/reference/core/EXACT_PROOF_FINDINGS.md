...
Phase 1 exit criterion ([ADR 0012 post-v6 addendum](../../adr/0012-edge-watershed-parity-bar.md#addendum-2026-07-12-post-v6-residual--network-is-the-open-ship-gate)): **evaluated** ADR 0012 on full `180709_E` for **both** Edges and Network. As of `canonical_full_v16` (2026-07-15 proofs on disk): Edges ✅ PASS evaluated (exact count **69,500 / 69,500**, ownership **99.999863%**); Network ❌ FAIL (strand multiset **48,048 / 48,049**). Strict-field connection equality is stretch once Network multisets pass. Operator brief: [.claude/HANDOFF.md](../../../.claude/HANDOFF.md).

**Proposal figures (standalone claim charts, not multipanel):** [figures/README.md](../../../figures/README.md).
KPI constants for regenerators live only in [`figures/parity_campaign_series.py`](../../../figures/parity_campaign_series.py) — update that file when metrics below move, then run `generate_parity_claim_figures.py`.

| Stage | Harness / prior work | Phase 1 certification bar (latest claim surface) |
| :--- | :--- | :--- |
| **Energy** | Native Hessian path exact-compatible | 🟢 `prove-exact --stage energy` vs **`180709_E_crop_M_v2`** / full `180709_E_full_v2` **PASS** (ADR 0011 `np.allclose`). |
| **Vertices** | Verified on prior surfaces | 🟢 **PASS** vs crop + full (exact positions/scales; energies under ADR 0011). |
| **Edges** | Watershed + finalization parity | 🟢 **ADR 0012 PASS** evaluated on `canonical_full_v16`: connections **69,500 / 69,500**, ownership **99.999863%**, trace failures **0**. |
| **Network** | Deterministic from edge set | ⛔ **ADR 0012 FAIL** on `canonical_full_v16`: strand endpoint-pair multiset **48,048 / 48,049**. Downstream of residual edge-set swap (no independent Network bug). **Open ship gate.** |
...
| **Crop stretch run** | `workspace/runs/oracle_180709_E/crop_M_exact_v3` | ✅ Edges rerun (2026-07-14 after MATLAB post-watershed finalization fix). Candidate generation **100%** (15,511 / 15,511); **19,225** raw candidates; generation gap **0**. Golden frontier trace **matches** MATLAB. Final selected edges **15,511** vs MATLAB **15,511**; final MATLAB-pair overlap **15,510** (missing **1**, extra **1**). Crop proof strict counts equal but ADR 0012 was not evaluated because the proof harness did not find a compatible ownership-map artifact from this resume path. |
...
1. **Crop generation and post-watershed finalization are effectively closed** — Crop-edge finalization now mirrors MATLAB `resample_vectors`, map-resampled size/energy, smoothing/crop unsigned casts, and cleanup on the resampled post-crop surface. Latest crop checkpoint candidates cover **15,511/15,511** MATLAB pairs (generation gap **0**) and raw watershed candidate pairs match MATLAB exactly (**19,225/19,225**). Final selected pairs are **15,511** vs MATLAB **15,511**, with **15,510/15,511** overlap; the remaining local issue is one equal-metric degree-pruning swap. Do not reopen Network as an independent port.
2. **Phase 1 ship gate = Network ADR 0012** — Edges ADR 0012 ✅ PASS evaluated on `canonical_full_v16` (69,500 / 69,500; ownership 99.999863%). Network ❌ FAIL on `v16` (`exact_proof_network.json`: strand multiset **48,048 / 48,049**), **entirely downstream of residual edge-set swap** (MATLAB-edge isolation is exact). Crop final residual is one equal-metric pair swap; triage that before another full launch.
