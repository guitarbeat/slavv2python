...
1. **Full Edge Set residual (generation join displacement)** — see top banner: extra watershed join `(26444→38584)` displaces oracle `(34897,38584)` at degree-excess under equal post-resample max; ablation drop-cand-`46698` → **69,500/69,500** pairs. Cleanup is faithful (MATLAB functions match Python on the same Candidate Set). **Production fix = join/emission**, not cleanup reorder.
2. **Phase 1 ship gate = Network ADR 0012 multiset** — Edges evaluated PASS on claim root `canonical_full_v16`; Network FAIL until Edge Set multiset matches (MATLAB-edge isolation exact). Do not reopen Network as an independent port.
3. **Crop / frontier / cleanup** — regression guards only (crop re-selection pair multiset closed; generation gap 0; golden trace match; cleanup comparator 0 mismatches).

**Superseded guidance:** “>95% match”, “block on 80% crop overlap”, “crop one-pair swap is the open loop”, or strict-field fallback as closure verdict. Only **evaluated** ADR 0012 on **both** Edges and Network count; see [ADR 0012 post-v6 addendum](../../adr/0012-edge-watershed-parity-bar.md#addendum-2026-07-12-post-v6-residual--network-is-the-open-ship-gate).
