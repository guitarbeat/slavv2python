...
| **Edges** | v29 ~88.7% pair match (baseline) | 🟡 Partially closed. Two MATLAB-faithful fixes (`edge_number_tolerance` 4→**2** per `get_edges_V300.m:100`; exact-route **conflict painting disabled** per the commented-out `choose_edges_V200`) moved Python **9,429 → 13,775** edges and shared pairs **5,109 → 8,135** (vs MATLAB 15,511; missing 7,376, extra 5,640). Materially improved but still ~52% shared. **Residual = frontier ordering** (Python `(energy, linear-index)` heap vs MATLAB's insertion-order `available_locations` list) — the hard remaining piece. |
