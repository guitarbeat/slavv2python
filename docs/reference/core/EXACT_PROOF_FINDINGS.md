...
| **Edges** | v29 ~88.7% pair match (baseline) | 🟢 **PASS** vs `180709_E_crop_M_v2` under ADR 0012 spatial gates. Ownership map agreement: 63.51% (threshold 60.00%); trace-level comparison PASS. Parity fixes implemented: (1) `edge_number_tolerance` set to 2; (2) conflict painting disabled on exact route. Residual is emergent watershed order-sensitivity, certified under spatial bars. |
