...
| **Edges** | Watershed parity fixes (2026-07-07), crop-truncation fix (2026-07-08), vertex `-Inf` sentinel fix + crop-axis finalization fix (2026-07-13) | 🟢 **ADR 0012 PASS** on full `180709_E_full_v2` (`canonical_full_v10`, 2026-07-13): `adr0012_evaluated: true`, ownership-map **99.9867%** (≥60% threshold). Python **70,247** vs MATLAB **69,500** (over by 747). Strict-field residual has flipped to over-selection. |
| **Network** | End-to-end pipeline runs | ⛔ **ADR 0012 FAIL** on full `180709_E_full_v2` (`canonical_full_v10`, 2026-07-13): strand endpoint-pair multiset mismatch — Python **48,583** vs MATLAB **48,049** strands (over by 534). **Entirely downstream of residual edge-set mismatch** (no independent network bug; isolated network with MATLAB edges reproduces exact topology). |

---

## 🚦 Active Phase 1 operations

> **Closure policy (2026-07-13 post-v10):** Phase 1 closes when a fresh full-volume run passes **evaluated** ADR 0012 for **Edges and Network**. **`canonical_full_v6`**, **`v7`**, **`v8`**, and **`v10`** are audit closure attempts: Edges ✅ Network ❌. Crop overlap ≥80% is **cleared**; current crop generation gap is **0**. See [HANDOFF](../../../.claude/HANDOFF.md).
