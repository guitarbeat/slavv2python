...
The `analytics/parity/` package is grouped into themed subpackages: **`proof/`** (the `coordinator`, `proofs`, comparators, `energy_proof_evidence`/`energy_ulp_proof`, `reports`/`counts`), **`runs/`** (`resume`, `jobs`, `monitor_daemon`, `preflight`, `writer_lease`, `bootstrap`, `execution`), **`oracle/`** (`surfaces`, `oracle_artifacts`, `promotion`, MATLAB/checkpoint loaders, `params_audit`, `models`), **`probes/`** (`adaptive_probes`, `trace_comparator`, `crop_export`, `edge_artifacts`, `matlab_fail_fast`), and **`cli_handlers/`** (the `cli_*` handler modules). At the package root stay `cli.py` (the `slavv parity` handler facade that re-exports every handler and hosts `handle_prove_exact_sequence`), `commands.py` (command registry), `constants.py`, and `utils.py`. `surfaces` is the dataset/oracle/run authority; `bootstrap` builds the init-exact-run layout; `proofs` delegates to the coordinator.
