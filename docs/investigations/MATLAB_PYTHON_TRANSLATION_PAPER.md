...
> **STATUS (DRAFT — IN PROGRESS):** The Energy and Vertices stages are **CERTIFIED** under the
> ADR 0011 tolerance gate (Accepted): strict zero on discrete/topological fields plus `np.allclose`
> on continuous float fields (cross-library float drift bounded at ~2×10⁻¹¹; see
> `docs/reference/core/EXACT_PROOF_FINDINGS.md`). **Edges are in progress; Network is pending.**
> Treat downstream parity claims below as targets/method until those stages certify.
