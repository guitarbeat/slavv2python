...
> **STATUS (DRAFT — IN PROGRESS):** Exact parity is a work in progress, not yet achieved.
> The Energy stage currently FAILS strict `np.equal` (cross-library float drift; see
> `docs/reference/core/EXACT_PROOF_FINDINGS.md`). Vertices/Edges/Network proofs are pending and ADR 0011 is pending.
> No stage is certified strict-zero — treat all parity claims below as targets/method, not completed facts.
