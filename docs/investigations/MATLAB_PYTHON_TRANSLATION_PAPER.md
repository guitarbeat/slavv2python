...
The method began as a zero-tolerance gate (`np.equal`, zero ULP). Cross-library BLAS and FFT non-associativity impose an irreducible numerical floor near $\epsilon_{\mathrm{BLAS}} \approx 2\times 10^{-11}$ on continuous fields. Consequently, the framework establishes a formal bifurcation:
...
A core challenge in edge discovery is the divergence between the static Hessian energy field $E_0(y, x, z)$ and the dynamic watershed-mutated claimed energy map $E_{\mathrm{claimed}}(y, x, z)$. During watershed flood-fill, voxels claimed by a vertex $v$ are penalized via geometric distance and directional suppression weights:

$$E_{\mathrm{claimed}}(p) = E_0(p) \cdot \exp\left(-\frac{1}{2} \left(\frac{3 \cdot d/r}{\tau_d}\right)^2\right) \cdot \frac{1 - \cos(\pi \cdot \min(1, \frac{4}{3} r/R))}{2} \cdot \max(0, \hat{u} \cdot \hat{v})$$
...
MATLAB and Python (NumPy/SciPy) implement mathematical primitives differently, and those differences compound nonlinearly through multi-stage pipelines.

* **Precision Collapsing**: MATLAB defaults to IEEE-754 `double` (64-bit). Downcasting intermediate watershed maps or vertex energies to `float32` collapsed adjacent floating-point values into single bins, altering seed priority in dense microvascular hubs.
...
* **Boundary Inf Propagation**: MATLAB `interp3` propagates $\infty$ boundary conditions across all immediate neighbors, suppressing spurious candidate seeds along volume edges. Standard SciPy interpolators (`scipy.interpolate.interpn`) discard or extrapolate edge values, requiring specialized interpolation kernels in Python (`_interp3_matlab_linear_inf`).
...
Python operates in C-order by default. To preserve deterministic hub expansion, the Exact Route enforces an internal $[Y, X, Z]$ grid with Fortran (F) memory order.
