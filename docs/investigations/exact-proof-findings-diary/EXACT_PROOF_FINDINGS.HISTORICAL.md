...
# Exact Proof Findings (archived snapshot)
...
- **Do not claim from `canonical_full_v17`:** mixed Energy rerun, missing local energy/vertices checkpoints, stale “running” snapshot, job “succeeded” on permission denied. New Edges→Network successor only.
- Scratch: `raw_full_candidates.mat`, `global_presort_candidates.mat`, `full_residual_pair_raw.npz`. Cheap tests: `tests/unit/pipeline/test_watershed_energy_map_sort_experiments.py`. Runbook: [raw-vs-final-candidate-compare.md](../../solutions/parity/raw-vs-final-candidate-compare.md).
...
**Dual hypothesis (2026-07-17) — resolved 2026-08-13:** H1 (MATLAB never emits the extra) is **false**. H2 is the residual, but the discriminator is MATLAB `sort_edges` on **claimed** `energy_map` max, not join-partner `find(...,'last')`. Spatial traces for the two pairs are identical (same 9 voxels). Do not re-open non-emission or a new join-rule rewrite.

Shared post-resample bottleneck for three hub edges is space `[30,295,51]` (metric −4.870…). Code for claimed-map sampling + `sort_edges` is in tree; **full Edges regen + evaluated proofs still required** (existing checkpoints still have original-field traces).
...
**Network one-strand residual (48,048 vs 48,049) — fix deployed, closure run in progress.**

**2026-08-11 tie-scan:** `_matlab_watershed_resolve_tied_join_partner` is **not** the residual
fix. Live MATLAB join is `min(current_strel_energies)` (L530). The only live
`find(..., 1, 'last')` is frontier insert (L567–568). Candidate count stayed 84,650
after the helper. Do not land tie-scan as the ship-gate change; it can perturb
generation. That helper and its kiro spec were removed.

**Do not claim:** “100% parity” or Phase 1 closed until a **new** (not `v17`) claim
...
| 2026-07-07 | **SortedFrontier** (faithful `available_locations` port) + insert tail-drop fix + MATLAB `min` strel argmin | **57.89%** (8,979 / 15,511); **17,253** candidates | Golden trace: **13,706 / 13,706** vertex `iteration_start` pops match MATLAB (1-based offset); first divergence at iteration **13,707** (post-vertex frontier). Harness: `scripts/edges/frontier_diff.py`. Default backend: `watershed_frontier_backend=sorted` (`heap` fallback). |
...
| 2026-07-14 | Bounded golden-trace regression after stale writer cleanup | **bounded_match** through iteration **30,000** | Killed two stale `edges/frontier_diff.py --regenerate-python` processes that were interleaving JSONL writes. Patched bounded comparison so `--stop-after-iteration` compares only iteration-bearing rows. Current bounded trace passes well beyond the retired 13,761 split. |
...
**Probe command:** `.\.venv\Scripts\python.exe scripts/edges/candidate_gap.py --run-dir workspace/runs/oracle_180709_E/crop_M_exact_v3 --oracle-root workspace/oracles/180709_E_crop_M_v2`

**80% milestone:** ✅ **cleared** (2026-07-07). **Current residual playbook:** [.claude/HANDOFF.md](../../../.claude/HANDOFF.md) § A (claimed-map `sort_edges` residual; crop = regression guard) → § B (successor full claim run after production fix). Do **not** re-open the retired crop one-pair loop as primary.

### 🔎 2026-07-04: Edge shortfall root cause (generation gap, not prune gap)

> **Historical diagnosis (2026-07-04).** Superseded as *current residual class* by [ONE TRUTH residual](#active-residual-why-network-is-red). Kept for investigation trail only.
...
5. **Residual loop (primary):** [ONE TRUTH residual](#active-residual-why-network-is-red). Crop frontier/generation/re-selection are **regression guards** (closed). Prefer `scripts/edges/selection_funnel.py` and `tests/unit/pipeline/test_watershed_energy_map_sort_experiments.py`.
...
> **Cleanup is faithful:** `edges/clean_edge_pairs_matlab.py` on full Python surface → **0** mismatches (pairs/degree/cycle). Do **not** add cleanup secondary-key hacks (breaks MATLAB≡Python on same surface; endpoint-descending already rejected).  
...
> **Ablation proof:** drop only candidate index **46698**, re-run `select_and_finalize_edge_set` → undirected pair overlap **69,500 / 69,500** (only_py=0, only_mat=0). That fakes MATLAB’s post-`sort_edges` rank. Live mechanism: [ONE TRUTH residual](#active-residual-why-network-is-red).  
