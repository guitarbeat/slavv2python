...
- **`slavv_python` layout:** Streamlit helpers live under `interface/streamlit/{views,services,state,launcher.py}`; Energy stretch isolation under `pipeline/energy/stretch/`; MATLAB Watershed Discovery ports under `pipeline/edges/watershed/`. Public `slavv` / `SlavvPipeline` imports unchanged.
- **Streamlit GUI audit G1–G18:** launch works (`views/` not MPA `pages/`); Home/About/Analysis copy matches Paper Path vs Exact Route; viz opacity/camera and Energy-slice controls actually apply; empty states share one helper; CuPy/SimpleITK Energy methods hide when missing; desktop curator is labeled and disable-able. Audit: [`2026-08-19-streamlit-gui-audit.md`](investigations/2026-08-19-streamlit-gui-audit.md).
