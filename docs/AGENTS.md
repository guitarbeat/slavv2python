...
- **Source of Truth:** This file is definitive for agent operating guidance, glossary, and repository conventions. Phase specs, live parity status, task checkboxes, and ADRs retain their own documented authority.
...
The single authoritative document for exact-route [Certification](#certification) on full `180709_E`: requirements and implementation together under `plans/phase-1-exact-route-spec.md`.
_Avoid_: Maintaining separate brainstorm and plan files for the same initiative; use `brainstorms/` only before the spec exists.

### Exact Proof Findings
The live status log for exact-parity work: active runs, `prove-exact` results, blockers, champion baselines, and a curated index of parity-related compound solutions under `reference/core/EXACT_PROOF_FINDINGS.md`.
...
│   │       │   ├── watershed_lut.py    # Watershed LUT geometry (production + parity)
│   │       │   ├── watershed_core.py   # FrontierQueue, VoxelClaimMap
...
│   │   │   ├── matlab_vector_loader.py # Oracle vector discovery and normalization
│   │   │   ├── python_checkpoint_loader.py # Python checkpoint normalization
│   │   │   ├── artifact_comparator.py  # Stage-by-stage exact artifact compare
│   │   │   ├── proof_report.py         # Human-readable prove-exact reports
│   │   │   ├── exact_proof_contract.py # Stage order and field contract
...
| Doc Index | [README.md](README.md) | Index for all maintained reference docs |
| MATLAB Parity Plan | [MATLAB_METHOD_IMPLEMENTATION_PLAN.md](reference/core/MATLAB_METHOD_IMPLEMENTATION_PLAN.md) | Claim boundaries, source-of-truth hierarchy, remaining work |
| MATLAB-to-Python Map | [MATLAB_PARITY_MAPPING.md](reference/core/MATLAB_PARITY_MAPPING.md) | Function-to-function mapping for exact parity |
| Exact Proof Findings | [EXACT_PROOF_FINDINGS.md](reference/core/EXACT_PROOF_FINDINGS.md) | Live parity status, active blockers, proof results, and cold-start protocol |
| Naming Guide | [PYTHON_NAMING_GUIDE.md](reference/workflow/PYTHON_NAMING_GUIDE.md) | Python naming conventions and package surfaces |
| Testing Guide | [tests/README.md](../tests/README.md) | Rules for test placement and markers |
| Parity Pre-Gate | [PARITY_PRE_GATE.md](reference/workflow/PARITY_PRE_GATE.md) | Three-tier pre-gate (synthetic → crop → canonical) |
| Parity Certification | [PARITY_CERTIFICATION_GUIDE.md](reference/workflow/PARITY_CERTIFICATION_GUIDE.md) | `prove-exact` / `prove-exact-sequence` on canonical volume |
| Documented Solutions | [solutions/](solutions/) | Searchable past fixes and workflows (`module`, `tags`, `problem_type` in YAML frontmatter); relevant when debugging parity, oracle promotion, or integration issues |
| ADRs | [adr/](adr/) | Architecture decisions (schema, executor, stage managers, parity coordinator) |
| Extraction Algorithms | [ADDING_EXTRACTION_ALGORITHMS.md](reference/workflow/ADDING_EXTRACTION_ALGORITHMS.md) | Contributor guide for new algorithms |
...
### Parity Experiments
Before continuing parity work, read `reference/core/EXACT_PROOF_FINDINGS.md`.
