...
- [~] 10. Launch canonical full-volume run (`180709_E`)
  ...
  - [~] 10.2 Launch or resume the canonical full-volume exact-route run
    ...
    - **Status (2026-07-04):** Canonical run COMPLETED (writer exited; all four `exact_proof_<stage>.json` written to `03_Analysis`). **Energy CERTIFIED** (0 scale mismatches / 16,777,216 voxels + allclose, ADR 0011) and **Vertices PASS** on the full volume. The reported **edges/network `passed:false` is a proof-MEASUREMENT artifact, NOT a full-volume algorithm bug** (root-caused 2026-07-04): the canonical oracle batch lacks the ADR 0012 ground truth `watershed_ownership_map.mat`, so `_compare_edges_adr0012_bar` silently fell back to STRICT `edges.connections` comparison and failed on a raw count mismatch (69,500 vs 60,213) — exactly the failure ADR 0012 says to ignore. The underlying edge divergence is the accepted greedy-watershed order-sensitivity and is scale-independent: endpoint-pair overlap is ~50.3% canonical vs ~50.7% crop (crop clears the tolerant ownership bar at 63.47% only because it HAS the map artifact; its `0/0` network is stage-isolated on MATLAB edges, whereas canonical network is end-to-end). Comparator now emits a self-documenting `edges_adr0012_gate` with `adr0012_evaluated:false` on this fallback (commit `55ac449d`). **No canonical rerun warranted.** To close canonical edges/network under ADR 0012: (1) supply canonical ownership-map ground truth (candidate: `visual_data/edges_*_vertex_index_map.tif`, after cross-checking TIFF↔`.mat` convention, OR run the instrumented MATLAB edges harness on the full volume), (2) re-capture Python candidate with `--include-debug-maps` (edges only), (3) re-run per-stage `prove-exact --stage edges` (ADR 0012) + network via stage-isolation. Diagnosis dumps: `03_Analysis/exact_mismatch_{edges,network}.json`; analysis script `workspace/scratch/edge_gap_analysis.py`.
