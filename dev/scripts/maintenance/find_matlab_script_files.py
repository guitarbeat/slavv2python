...
def find_matlab_script_files() -> list[Path]:
    matlab_files = sorted(
        path for root in MATLAB_SEARCH_ROOTS for path in root.rglob("*.m") if path.is_file()
    )
...
def main() -> None:
    ...
    for path in find_matlab_script_files():
