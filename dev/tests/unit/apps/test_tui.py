...
from source.apps.cli.tui import is_tui_available, run_monitor_if_supported, run_wizard_if_supported
...
@patch("source.apps.cli.tui._TUI_AVAILABLE", False)
...
@patch("source.apps.cli.tui._TUI_AVAILABLE", True)
@patch("source.apps.cli.tui.wizard.run_setup_wizard")
...
@pytest.mark.skipif(not is_tui_available(), reason="TUI extra is not installed")
@patch("source.apps.cli.tui._TUI_AVAILABLE", True)
@patch("source.apps.cli.tui.runner_app.SLAVVPipelineApp")
...
def test_textual_app_reactive_states():
    ...
    from source.apps.cli.tui.runner_app import SLAVVPipelineApp
