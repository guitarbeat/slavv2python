...
def test_promote_dataset_copies_input_and_writes_manifest(tmp_path):
    ...
    manifest = json.loads(
        (dataset_root / DATASET_MANIFEST_PATH).read_text(encoding="utf-8")
    )
    ...
    assert (dataset_root / DATASET_INPUT_DIR / dataset_file.name).read_bytes() == (
        b"tiff-payload"
    )
    assert (
        dataset_root / DATASET_INPUT_DIR / f"{dataset_file.name}.sha256"
    ).is_file()
    index_lines = (
...
def test_promote_oracle_writes_manifest_and_index(tmp_path):
    ...
    manifest = json.loads(
        (oracle_root / ORACLE_MANIFEST_PATH).read_text(encoding="utf-8")
    )
...
def test_promote_report_copies_analysis_and_writes_manifest(tmp_path):
    ...
    report_manifest = json.loads(
        (report_root / REPORT_MANIFEST_PATH).read_text(encoding="utf-8")
    )
