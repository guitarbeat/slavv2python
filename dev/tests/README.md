...
Keep tests under `dev/tests/` organized by the owning package surface, not by the
historical task that introduced them.

## Placement Rules

- `dev/tests/unit/<owner>/` for package behavior owned by a source package such as
  `analysis`, `utils`, `io`, `core`, or `apps`
- `dev/tests/unit/workspace_scripts/` for maintained helper scripts that live under
  `dev/scripts/`
- `dev/tests/unit/parity/` for pure-Python parity helpers, comparison layout, and
  comparison runtime logic
- `dev/tests/integration/` for cross-component workflows and end-to-end pipeline
  paths
- `dev/tests/ui/` for Streamlit- and visualization-facing behavior
- `dev/tests/diagnostic/` for environment checks, MATLAB availability, and
