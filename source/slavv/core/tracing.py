...
def trace_edge(
    ...
    # Decompose position and direction into scalars for performance
    ...
    inv_mpv_2x_0 = inv_mpv_2x[0]
    inv_mpv_2x_1 = inv_mpv_2x[1]
    inv_mpv_2x_2 = inv_mpv_2x[2]

    # Image dimensions for bounds checking
    sy, sx, sz = energy.shape

    p0 = int(math.floor(cx))
    p1 = int(math.floor(cy))
    p2 = int(math.floor(cz))

    # Initial energy (assuming start_pos is valid/in-bounds)
    prev_energy = energy[p0, p1, p2]

    for _ in range(max_steps):
        attempt = 0
        # Variables to store next position
        ...
        while attempt < 10:
            ...
            if discrete_steps:
                ...
                nz = round(nz)
                # Check if we moved
                if nx == round(cx) and ny == round(cy) and nz == round(cz):
                    return trace
        ...
        if not valid_step:
            # Should have returned inside the loop if failed
            return trace
        ...
        # Append to trace (requires array creation)
        current_pos_arr = np.array([cx, cy, cz])
        ...
        gp0 = p0
        if gp0 < 1: gp0 = 1
        elif gp0 > sy - 2: gp0 = sy - 2

        gp1 = p1
        if gp1 < 1: gp1 = 1
        elif gp1 > sx - 2: gp1 = sx - 2

        gp2 = p2
        if gp2 < 1: gp2 = 1
        elif gp2 > sz - 2: gp2 = sz - 2
