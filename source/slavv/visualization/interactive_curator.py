...
from PyQt5.QtWidgets import (
    ...
    QSplitter, QSlider, QLabel, QPushButton, QComboBox, QGroupBox, QGridLayout
)
...
class InteractiveCurator(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.energy_line = pg.InfiniteLine(pos=0, angle=90, movable=True, pen=pg.mkPen('r', width=2))
    ...
    def _init_data(self):
        ...
        self.hist_intensity.plot(x_int, y_int, stepMode="center", fillLevel=0, brush=(200, 200, 200, 180))
        ...
        if 'energies' in self.vertices_data and len(self.vertices_data['energies']) > 0:
            ...
            self.hist_energy.plot(x_eng, y_eng, stepMode="center", fillLevel=0, brush=(0, 120, 255, 150))
    ...
    def _update_views(self):
        ...
        if len(positions) > 0:
            ...
            if len(vis_pos) > 0:
                ...
                self.plotter_display.add_mesh(
                    poly, scalars="status", cmap=["red", "blue"],
                    point_size=12, render_points_as_spheres=True,
                    show_scalar_bar=False, name='vertices',
                )
        ...
        if len(traces) > 0:
            ...
            if pts_list:
                ...
                self.plotter_display.add_mesh(
                    edge_poly, scalars="status", cmap=["red", "cyan"],
                    line_width=3, show_scalar_bar=False, name='edges',
                )
        ...
        # ── Add-edge mode (2-click) ───────────────────────────────────────
            ...
            if dists[idx] < 5.0:
                ...
                if len(self._add_edge_buffer) == 2:
                    ...
                    trace = [list((p1 + (p2 - p1) * (s / (n_steps - 1))).round().astype(int))
