...
def plot_radius_distributions(
    ...
    all_radii = (
        np.concatenate([matlab_radii, python_radii])
        if (len(matlab_radii) > 0 or len(python_radii) > 0)
        else []
    )
    ...
    if len(all_radii) > 0:
        ...
        if len(matlab_radii) > 0:
            ax.hist(
                ...
                bins=bins,
                alpha=0.6,
                label=f"MATLAB (n={len(matlab_radii)})",
                color="#2E86AB",
            )
        if len(python_radii) > 0:
            ax.hist(
                ...
                bins=bins,
                alpha=0.6,
                label=f"Python (n={len(python_radii)})",
                color="#A23B72",
            )
