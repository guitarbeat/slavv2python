...
from slavv.evaluation.viz import plot_count_comparison, set_plot_style
...
BOX_LAYOUT = widgets.Layout(
    border="1px solid #e0e0e0", padding="15px", margin="10px 0", border_radius="8px", width="95%"
)
...
def find_available_files(project_root: Path) -> list[str]:
    ...
    search_paths = [project_root / "data", project_root / "tests" / "data"]
    available_files = []
...
class BaseRunnerWidget:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        if self.project_root.name in [
            "scripts",
            "notebooks",
        ]:  # Handle if running from scripts or notebooks dir
    ...
    def _create_file_box(self):
        return widgets.VBox([create_header("File Input"), self.input_widget], layout=BOX_LAYOUT)
...
class MatlabRunnerWidget(BaseRunnerWidget):
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        self.default_output = str(
            self.project_root / "experiments" / f"{self.timestamp}_matlab_run"
        )
    ...
    def _create_path_box(self):
        return widgets.VBox(
            [create_header("System Paths"), self.matlab_widget, self.output_widget],
            layout=BOX_LAYOUT,
        )
    ...
    def display(self):
        """Display the widget."""
        ui = widgets.VBox(
            [self._create_file_box(), self._create_path_box(), self.run_button, self.output_area]
        )
        display(ui)
...
class PythonRunnerWidget(BaseRunnerWidget):
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        # specific defaults
        self.default_output = str(
            self.project_root / "experiments" / f"{self.timestamp}_python_run"
        )
    ...
    def _create_path_box(self):
        return widgets.VBox([create_header("System Paths"), self.output_widget], layout=BOX_LAYOUT)
...
class ComparisonDashboardWidget:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        # Find runs
        self.all_runs = sorted(
            [d.name for d in self.comparisons_dir.iterdir() if d.is_dir()], reverse=True
        )

        # Heuristics
        self.matlab_candidates = [r for r in self.all_runs if "matlab" in r.lower()] + [
            r for r in self.all_runs if "matlab" not in r.lower()
        ]
        self.python_candidates = [r for r in self.all_runs if "python" in r.lower()] + [
            r for r in self.all_runs if "python" not in r.lower()
        ]
...
def show_comparison_plots(report_path: Optional[Path] = None):
    ...
    if total_elements == 0:
        display(
            HTML(
                "<div style='background-color: #fff3cd; color: #856404; padding: 15px; border: 1px solid #ffeeba; border-radius: 4px;'><strong>Warning:</strong> The comparison data contains zero elements. Please check that your source runs produced valid output.</div>"
            )
        )
        return
