...
def test_vertices_rendering(mock_network_data):
    # Verify that enabling vertices doesn't crash (regression test for x_axis name error)
    vertices, edges, network, parameters = mock_network_data
    viz = NetworkVisualizer()

    # Run with vertices enabled
    fig = viz.plot_3d_network(
        vertices, edges, network, parameters,
        show_vertices=True,
        show_edges=True
    ...
    # Check if vertices trace exists
    vertex_traces = [t for t in fig.data if t.mode == 'markers' and t.name == 'Vertices']
    assert len(vertex_traces) > 0
