...
def test_pipeline_result_roundtrip_preserves_nested_payloads_and_extra_fields():
    payload = build_processing_results(overrides={"metadata": {"slavv_python": "unit-test"}})
    ...
    assert restored["metadata"] == {"slavv_python": "unit-test"}


def test_normalize_pipeline_result_accepts_mapping_like_payloads():
    payload = build_processing_results(overrides={"metadata": {"slavv_python": "mapping"}})
    ...
    assert result.extra["metadata"] == {"slavv_python": "mapping"}
