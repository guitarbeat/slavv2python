...
from slavv_python.workflows.pipeline.results import (
...
def test_finalize_pipeline_results_returns_plain_dict_payload():
    payload = build_processing_results(overrides={"metadata": {"slavv_python": "workflow"}})

    finalized = finalize_pipeline_results(payload)

    assert finalized["metadata"] == {"slavv_python": "workflow"}
