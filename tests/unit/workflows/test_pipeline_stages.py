...
from slavv_python.workflows.pipeline_stages import (
    resolve_edges_stage,
...
def test_resolve_stage_with_checkpoint_uses_fallback_without_run_context():
    result = resolve_stage_with_checkpoint(
        ...
        success_detail="Done",
        fallback_fn=lambda: {"slavv_python": "fallback"},
        compute_fn=lambda: {"slavv_python": "compute"},
        logger=logging.getLogger("test-pipeline-stages"),
    )

    assert result == {"slavv_python": "fallback"}
...
def test_resolve_stage_with_checkpoint_uses_resumable_helper(monkeypatch):
    ...
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_stages.resolve_resumable_stage",
    ...
    result = resolve_stage_with_checkpoint(
        ...
        compute_fn=lambda stage_controller: (
            compute_calls.append(stage_controller) or {"slavv_python": "compute"}
        ),
        logger=logging.getLogger("test-pipeline-stages"),
    )

    assert result == {"slavv_python": "compute"}
...
def test_resolve_stage_with_checkpoint_marks_stage_failed_before_reraising(monkeypatch):
    ...
    monkeypatch.setattr("slavv_python.workflows.pipeline_stages.resolve_resumable_stage", boom)
...
def test_resolve_energy_stage_uses_standard_stage_metadata(monkeypatch):
    ...
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_stages.resolve_stage_with_checkpoint",
    ...
    result = resolve_energy_stage(
        ...
        force_rerun=True,
        fallback_fn=lambda: {"slavv_python": "fallback"},
        resumable_fn=lambda controller: {"slavv_python": "resumable"},
        logger=logging.getLogger("test-pipeline-stages"),
    )
