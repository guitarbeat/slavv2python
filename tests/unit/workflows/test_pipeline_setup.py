...
from slavv_python.workflows.pipeline_setup import (
    PreparedPipelineRun,
    create_run_context,
    effective_run_dir,
    emit_progress,
...
class _DummyRunContext:
    def __init__(self, **kwargs) -> None:
        run_dir = kwargs.get("run_dir", "run-root")
...
def test_initialize_run_context_writes_params_and_marks_running(monkeypatch):
    ...
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.atomic_write_json",
...
def test_preprocess_image_marks_completion(monkeypatch):
    ...
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.utils.preprocess_image",
...
def test_preprocess_image_marks_failure_before_reraising(monkeypatch):
    ...
    monkeypatch.setattr("slavv_python.workflows.pipeline_setup.utils.preprocess_image", boom)
...
def test_prepare_pipeline_run_validates_inputs_and_initializes_context(monkeypatch, tmp_path):
    run_context = _DummyRunContext(run_dir=str(tmp_path / "run"))

    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.utils.validate_parameters",
        lambda params: {"validated": True, **params},
    )
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.fingerprint_array",
        lambda image: "image-fingerprint",
    )
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.fingerprint_jsonable",
        lambda params: "params-fingerprint",
    )
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.effective_run_dir",
        lambda run_dir, event_callback: str(tmp_path / "run"),
    )
    ...
    monkeypatch.setattr(
        "slavv_python.workflows.pipeline_setup.create_run_context", fake_create_run_context
    )
    ...
    assert create_calls == [
        {
            "effective_run_dir_value": str(tmp_path / "run"),
