...
from .curation.machine_learning import MLCurator, DrewsCurator, extract_uncurated_info
...
def _ensure_dict(obj, default_keys, primary_key):
    ...
    if isinstance(obj, np.ndarray):
        ...
        lengths = [len(v) for v in d.values() if hasattr(v, "__len__") and not isinstance(v, (str, bytes))]
...
def choose_edges(edges, vertices=None, parameters=None, **kwargs):
    ...
    vertices_dict = _ensure_dict(vertices, {
        "positions": np.zeros((0, 3), dtype=float),
        "original_indices": np.array([], dtype=int),
    }, "positions")

    return AutomaticCurator().curate_edges_automatic(edges_dict, vertices_dict, p)["original_indices"]
...
def choose_vertices(vertices, energy_data=None, parameters=None, **kwargs):
    ...
    return AutomaticCurator().curate_vertices_automatic(vertices_dict, energy_data, p)["original_indices"]
