...
from .global_watershed import _generate_edge_candidates_matlab_global_watershed as execute_watershed_engine
...
def _trace_vertex_unit(
    ...
    **kwargs: Any
    ...
    directions = _get_seed_directions(vertex_idx, start_pos, start_radius, energy, mpv, facade, params)
...
def _get_seed_directions(
    ...
    if method == "hessian":
        dirs = facade.estimate_vessel_directions(energy, pos, r, mpv, facade.generate_edge_directions)
        if len(dirs) < limit:
...
def _assemble_parallel_results(results: list[tuple]) -> dict[str, Any]:
    ...
    candidates = {
        "traces": [], "connections": [], "metrics": [], 
        "energy_traces": [], "scale_traces": [], "origin_indices": [],
        "candidate_source": "fallback", "diagnostics": _empty_edge_diagnostics()
    }
