...
def categorize_missing_pairs(
    missing_pairs: list[list[int]], matlab_vertices: set[int] | None = None
) -> dict[str, Any]:
    ...
    return {
        ...
        "top_missing_vertices": [
            {"vertex": v, "pair_count": count} for v, count in vertex_counter.most_common(20)
        ],
        "distance_stats": {
        ...
        "connectivity_patterns": {
            ...
            "isolated_vertices_sample": sorted(isolated_vertices)[:10],
            "hub_vertices": sorted(
                [{"vertex": v, "degree": vertex_degree[v]} for v in hub_vertices],
...
def categorize_extra_pairs(
    extra_pairs: list[list[int]], python_vertices: set[int] | None = None
) -> dict[str, Any]:
    ...
    return {
        ...
        "connectivity_patterns": {
            "vertex_degrees": sorted(
                [{"vertex": v, "degree": vertex_degree[v]} for v in all_vertices],
                key=lambda x: x["degree"],
                reverse=True,
            )[:10],
        },
...
def investigate_baseline_discrepancy(
    experiment_root: Path, current_match_rate: float
) -> dict[str, Any]:
...
def identify_root_causes(
    missing_analysis: dict[str, Any], extra_analysis: dict[str, Any]
) -> dict[str, Any]:
...
def main():
    ...
    # Paths
    experiment_root = REPO_ROOT / "workspace" / "comparisons" / "experiments" / "live-parity"
    ...
    print(
        f"Match rate: {coverage['matched_pair_count'] / coverage['matlab_pair_count'] * 100:.1f}%"
    )
    print()
    ...
    print(
        f"  Isolated vertices: {missing_analysis['connectivity_patterns']['isolated_vertex_count']}"
    )
    print(
        f"  Hub vertices (degree >=5): {missing_analysis['connectivity_patterns']['hub_vertex_count']}"
    )
    print()
    ...
    output_dir = REPO_ROOT / "workspace" / "tmp_tests" / "investigations"
