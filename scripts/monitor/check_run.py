...
def main() -> None:
    ...
    parser.add_argument(
        "--stall-min", type=float, default=15.0, help="Heartbeat age (min) to flag STALLED"
    )
    ...
    checkpoints = (
        {p.stem.replace("checkpoint_", "") for p in ckpt_dir.glob("checkpoint_*.pkl")}
        if ckpt_dir.is_dir()
        else set()
    )
    ...
    if exit_code not in (None, 0) or job.get("reason"):
        verdict = "FAILED"
    elif job_status in {"completed", "succeeded"} or (
        all_done and snapshot.get("status") == "completed"
    ):
        verdict = "COMPLETED"
    elif (
        job_status == "running"
        and heartbeat_age is not None
        and heartbeat_age > args.stall_min * 60
    ):
        verdict = "STALLED"
    ...
    for s in _STAGES:
        ...
        mark = (
            "[x]"
            if s in checkpoints
            else {"running": "[>]", "completed": "[x]"}.get(str(st.get("status")), "[ ]")
        )
    ...
    if resume.get("units_total"):
        ...
        print(
            f"energy octave {resume.get('octave')}: merge cursor {u}/{t} ({100 * u / t:.1f}%) -- lags compute under n_jobs>1"
        )
