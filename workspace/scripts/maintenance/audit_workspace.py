...
# Add source to path if running from workspace/scripts/maintenance
repo_root = Path(__file__).resolve().parents[3]
if str(repo_root) not in sys.path:
    sys.path.insert(0, str(repo_root))

try:
    from slavv_python.runtime.workspace import WorkspaceAuditor
except ImportError:
    print("Error: Could not import slavv_python.runtime.workspace. Are you running from the repo root?")
    sys.exit(1)
