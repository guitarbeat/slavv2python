...
from workspace.scripts.cli.parity_experiment import main
from workspace.tests.support.run_state_builders import (
...
#     matching_cmdline = [
#         "python.exe",
#         "workspace/scripts/cli/parity_experiment.py",
#         "fail-fast",
#         "--dest-run-root",
#         normalized_dest,
#     ]
...
def test_capture_candidates_persists_heartbeat_detail(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "slavv_python.analysis.parity.proofs._load_exact_vertices_payload",
        lambda _surface: {
    ...
    monkeypatch.setattr(
        "slavv_python.core.edge_candidates._generate_edge_candidates_matlab_frontier",
        fake_generate,
    )
    monkeypatch.setattr(
        "slavv_python.core.edge_candidates._finalize_matlab_parity_candidates",
        lambda candidate_payload, *_args: candidate_payload,
    )
    monkeypatch.setattr(
        "slavv_python.analysis.parity.proofs.load_normalized_matlab_vectors",
    ...
    monkeypatch.setattr(
        "slavv_python.analysis.parity.proofs._load_matlab",
        lambda *_args, **_kwargs: {
            "edges": {
                "connections": np.array([[0, 0]], dtype=np.int32),
            }
    ...
    monkeypatch.setattr(
        "slavv_python.io.matlab_fail_fast.build_candidate_coverage_report",
