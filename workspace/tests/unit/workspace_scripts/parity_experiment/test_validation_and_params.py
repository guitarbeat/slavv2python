...
import pytest
from slavv_python.analysis.parity.constants import CHECKPOINTS_DIR
...
def test_validate_source_run_surface_accepts_required_artifacts(tmp_path):
    ...
    assert surface.checkpoints_dir == run_root.resolve() / CHECKPOINTS_DIR
...
def test_validate_source_run_surface_reports_missing_artifacts(tmp_path):
    ...
    with pytest.raises(ValueError, match="missing required artifacts"):
        validate_source_run_surface(run_root)
...
def test_load_params_file_uses_source_default_and_override(tmp_path):
    ...
    default_params = load_params_file(surface, None)
    override_params = load_params_file(surface, str(override))
...
def test_load_params_file_rejects_python_only_parity_controls_on_exact_route(tmp_path):
    ...
    with pytest.raises(ValueError, match="disallowed Python-only parity keys"):
        load_params_file(surface, str(override))
...
#     surface = validate_exact_proof_source_surface(run_root)
...
#         validate_exact_proof_source_surface(run_root)
...
#     with pytest.raises(ValueError, match="disallowed Python-only parity keys"):
#         load_exact_params_file(surface)
