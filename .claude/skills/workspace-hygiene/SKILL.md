...
4. Route clutter:
   ...
   - Ephemeral agent harness output (`agent-tools/`, `terminals/`, recreated `.agents/`) must not live at the repo root; move to `workspace/scratch/` or delete. Canonical agent home is `.claude/` (do not recreate `.agents/` or `.kiro/`).
